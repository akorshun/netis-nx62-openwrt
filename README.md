...
Версию проще всего узнать по размеру блока NAND: `cat /sys/class/mtd/mtd0/erasesize` покажет `131072` у стандартной версии и `262144` у версии с 512 МБ. Скрипты проверяют это сами: если запустить не тот, он остановится и подскажет нужную команду.
...
`flash-512m.sh` для версии с NAND Toshiba на 512 МБ (блок 256 КБ, страница 4 КБ):

1. записывает кастомный загрузчик **U-Boot 2025.07-WildEdition**: BL2 в раздел `bl2`, BL31 + U-Boot в раздел `fip`;
...
1. Проверяет модель и NAND: блок 256 КБ, страница 4 КБ, стандартное начало разметки (`bl2`, `u-boot-env`, `factory`, `fip`), нет bad-блоков в `bl2` и `fip`.
2. Скачивает BL2 и U-Boot из этого репозитория (запасной источник — jsDelivr) и сверяет SHA-256.
