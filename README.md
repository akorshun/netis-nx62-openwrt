...
BL2 WildEdition собран только под страницу 4 КБ: BootROM не прочитает его с микросхемы со страницей 2 КБ, и роутер не загрузится. Официальный BL2 грузится на `W25N04KV` (проверено на роутере). Сам U-Boot WildEdition эту микросхему знает. **Связку «официальный BL2 + U-Boot WildEdition» на Winbond ещё никто не проверял.** Первый раз запускайте её, приготовив UART-восстановление.
